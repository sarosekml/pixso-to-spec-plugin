...
test("HTML export renders every selected frame as a JPEG data URI", async () => {
  const page = { id: "0:1", type: "PAGE" };
  ...
  assert.equal(rows[0].row.imageDataUri, "data:image/jpeg;base64,AQID");
  assert.equal(rows[1].row.imageDataUri, "data:image/jpeg;base64,BAUG");
  ...
  assert.equal(runtime.messages.some((message) => message.type === "export-complete"), true);
});
...
test("preferences are normalized and persisted through Pixso storage", async () => {
  const runtime = await loadPlugin({ savedPreferences: { format: "html", theme: "dark" } });
  ...
  await runtime.pixso.ui.onmessage({ type: "save-preferences", format: "invalid", theme: "LIGHT" });
